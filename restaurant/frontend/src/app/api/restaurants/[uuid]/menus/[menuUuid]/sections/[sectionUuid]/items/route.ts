...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(
      `${API_URL}/restaurants/${uuid}/menus/${menuUuid}/sections/${sectionUuid}/items`,
      {
    ...
    return NextResponse.json(
      { message: 'Не вдалося отримати позиції розділу' },
      { status: 502 },
    );
  }
}
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json(
      { message: 'Не вдалося оновити позиції в розділі' },
      { status: 502 },
    );
  }
}
